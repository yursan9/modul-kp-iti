...
Nilai dalam \eng{array} diakses dengan memanggil nama variabel ditambah indeks penyimpanan \eng{array}. Indeks \eng{array} dalam C++ dimulai dari \(0\). Jika terdapat \eng{array} dengan \(5\) kolom, indeks terakhir adalah \(4\).
...
Program di bawah mencontohkan cara penggunaan \eng{array} untuk menyimpan data yang dimasukkan dan menampilkan data tersebut.

\cppfile{code/contoh_array.cpp}

\section{Array n-dimensi}
\eng{Array} juga bisa digunakan untuk menyimpan data yang terdiri dari kolom dan baris (\eng{array} \(2\) dimensi), ataupun \(n\)-dimensi. Contoh pembuatan \eng{array} 2 dimensi;
